...
from common.constants import (
    AWS_REGION, AWS_ACCESS, AWS_SECRET, END_TIME, MAIN_VIDEO_BUCKET, MAIN_VIDEO_DESTINATION, NEED_TRIM, START_TIME,
    TEMP_VIDEO_BUCKET, TEMP_VIDEO_DESTINATION,
...
class BatchMainSampleVideo(Path):
    ...
    def _exec(self):
        ...
        if NEED_TRIM: # trim sample video
            ...
            if sample_video_path is None:
                return
            sample_upload_done = self.__upload_sample_video(SAMPLE_VIDEO_BUCKET, SAMPLE_VIDEO_DESTINATION)
            if sample_upload_done:
                self.logger.info(f"Sample video upload success to {SAMPLE_VIDEO_BUCKET}/{SAMPLE_VIDEO_DESTINATION}")
            else:
                self.logger.error(f"Sample video upload failed to {SAMPLE_VIDEO_BUCKET}/{SAMPLE_VIDEO_DESTINATION}")
        main_upload_done = self.__upload_main_video(MAIN_VIDEO_BUCKET, MAIN_VIDEO_DESTINATION, video_path)
        if main_upload_done:
            self.logger.info(f"Main video upload success to {MAIN_VIDEO_BUCKET}/{MAIN_VIDEO_DESTINATION}")
        else:
            self.logger.error(f"Main video upload failed to {MAIN_VIDEO_BUCKET}/{MAIN_VIDEO_DESTINATION}")
        return

    def __cut_video(self, video_path: str) -> str:
        try:
            self.logger.info(f"Cut video from {START_TIME} to {END_TIME} duration {END_TIME - START_TIME} {video_path}")
            cmd = [
                "ffmpeg",
                "-i", video_path,
                "-ss", str(START_TIME),
    ...
    def __download_temp_video(self) -> str:
        try:
            ...
            self.s3_client.download_file(TEMP_VIDEO_BUCKET, TEMP_VIDEO_DESTINATION, f"{BatchMainSampleVideo.TEMP_VIDEO_PATH}/temp_video.mp4")
    ...
    def __upload_main_video(self, bucket: str, destination: str, video_path: str) -> bool:
        try:
            self.s3_client.upload_file(
                video_path, 
                bucket, 
