...
class Payments(Base):
    ...
    provider_payment_id: Mapped[str] = mapped_column(
        Text, nullable=False, comment="Credix決済ID（session_id）"
    )
    ...
    payment_amount: Mapped[int] = mapped_column(
        BigInteger,
        nullable=False,
        comment="支払総額（buyer_user_idが実際に支払った金額 = item_price + purchase_fee）",
    )
    payment_price: Mapped[int] = mapped_column(
        BigInteger, nullable=False, comment="支払い金額（price or plan.price）"
    )
    ...
    paid_at: Mapped[Optional[datetime]] = mapped_column(
        nullable=True, index=True, comment="実際の決済完了日時"
    )
    ...
    platform_fee: Mapped[int] = mapped_column(
        SmallInteger, nullable=False, comment="その時のプラットフォーム手数料"
    )
