...
def delete_like(db: Session, user_id: UUID, post_id: UUID) -> bool:
    ...
    like = (
        db.query(Likes)
        .filter(and_(Likes.user_id == user_id, Likes.post_id == post_id))
        .first()
    )
...
def is_liked(db: Session, post_id: UUID, current_user: Users = None) -> bool:
    """
    ...
    if current_user is not None:
        like = (
            db.query(Likes)
            .filter(
                and_(Likes.user_id == str(current_user.id), Likes.post_id == post_id)
            )
            .first()
        )
    else:
...
def add_notification_like(db: Session, user_id: UUID, post_id: UUID) -> None:
    ...
    try:
        ...
        display_name = (
            liked_user.profile_name
            if liked_user.profile_name
            else (liked_user.email or "ユーザー")
        )
        avatar_url = (
            f"{os.environ.get('CDN_BASE_URL')}/{liked_user_profile.avatar_url}"
            if (liked_user_profile and liked_user_profile.avatar_url)
...
def add_mail_notification_like(db: Session, user_id: UUID, post_id: UUID) -> None:
    ...
    try:
        user = (
            db.query(Users, Profiles, Posts, UserSettings.settings)
