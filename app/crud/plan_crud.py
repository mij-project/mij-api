...
def get_plan_by_user_id(db: Session, user_id: UUID) -> dict:
    ...
    for subscription in subscribed_subscriptions:
        # order_idからプランIDを取得
        try:
            plan_id = UUID(subscription.Subscriptions.order_id)
        ...
        plan_price = subscription.Payments.payment_amount
        ...
        subscribed_plan_details.append(
            {
                "purchase_id": str(subscription.Subscriptions.id),
                "plan_id": str(plan.id),
                "plan_name": plan.name,
                "plan_description": plan.description,
                "price": plan_price,
                "purchase_created_at": subscription.Subscriptions.created_at,
                ...
                "creator_username": creator_profile.username
                if creator_profile
                else None,
                "creator_profile_name": creator_profile.profile_name
                if creator_profile
                else None,
...
def get_plan_detail(db: Session, plan_id: UUID, current_user_id: UUID) -> dict:
    ...
    # プラン情報を取得
    plan = (
        db.query(Plans).filter(Plans.id == plan_id, Plans.deleted_at.is_(None)).first()
    )

    if not plan:
        return None
    is_time_sale = False
    plan_time_sale_info = None
    plan_time_sale = get_active_plan_timesale(db, plan.id)
    if (
        plan_time_sale
        and plan_time_sale["is_active"]
        and (not plan_time_sale["is_expired"])
    ):
        is_time_sale = True
        plan_time_sale_info = plan_time_sale
    ...
    return {
        ...
        "creator_avatar_url": f"{BASE_URL}/{creator_info.avatar_url}"
        if creator_info and creator_info.avatar_url
        else None,
        "creator_cover_url": f"{BASE_URL}/{creator_info.cover_url}"
        if creator_info and creator_info.cover_url
        else None,
        "post_count": post_count or 0,
        "is_subscribed": is_subscribed,
        "subscriptions_count": subscriptions_count,
        "is_time_sale": is_time_sale,
        "time_sale_info": plan_time_sale_info,
        "plan_post": [
            {
...
def get_plan_posts_paginated(
    db: Session, plan_id: UUID, current_user_id: UUID, page: int = 1, per_page: int = 20
):
    """
    ...
    posts_query = (
        ...
        .outerjoin(
            ThumbnailAssets,
            (ThumbnailAssets.post_id == Posts.id)
    ...
    price_pairs = [(row.Posts.id, row.price_id) for row in posts_query]
    ...
    for (
        ...
        end_date = None
        is_time_sale = False
        ts_found = [x for x in active_price_pairs if x[0] == str(post.id)]

        if ts_found:
            is_time_sale = True
            sale_percentage = ts_found[0][2]
            end_date = ts_found[0][3]
...
def get_plan_subscribers_paginated(
    db: Session, plan_id: UUID, page: int = 1, per_page: int = 20
) -> dict:
    """
...
def get_creator_posts_for_plan(
    db: Session, creator_user_id: UUID, plan_id: Optional[UUID] = None
) -> List[dict]:
    """
    ...
    if plan_id:
        included_posts = (
            db.query(PostPlans.post_id).filter(PostPlans.plan_id == plan_id).all()
        )
...
def get_single_purchases_by_user_id(
    db: Session, user_id: UUID
) -> List[SinglePurchaseResponse]:
    """
    ユーザーの単品購入一覧を取得
    ...
    for purchase in purchases_query:
        single_purchases.append(
            SinglePurchaseResponse(
                ...
                purchase_price=int(purchase.purchase_price)
...
def get_plan_monthly_sales(db: Session, creator_user_id: UUID) -> int:
    ...
    subquery = (
        ...
        .filter(
            ...
            == PaymentTransactionType.SUBSCRIPTION,  # 1=SUBSCRIPTION
        ...
        db.query(
            func.sum(
                func.floor(
                    ...
                    - (Payments.payment_price * Payments.platform_fee / 100)
                )
            )
        )
        .join(
