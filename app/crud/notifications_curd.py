...
from app.utils.trigger_batch_admin_notification import trigger_batch_admin_notification
...
def get_notifications_admin_paginated(
    ...
    query = db.query(Notifications).filter(
        Notifications.type == NotificationType.ADMIN, Notifications.user_id.is_(None)
    )
    if search:
...
def create_notification_admin(
    ...
    try:
        ...
        trigger_batch_admin_notification(str(new_notification.id))
...
def update_notification_admin(
    ...
    try:
        ...
        update_values = {
            "updated_at": datetime.now(timezone.utc),
            "target_role": notification_update.target_role,
        }
        if update_fields:
            pairs: list[object] = []
        ...
        if type == NotificationType.ADMIN:
            query = (
                ...
                .filter(
                    ...
                    or_(
                        Notifications.target_role.in_(target_role),
                        Notifications.target_role.is_(None),
                    ),
                    # Notifications.target_role.in_(target_role),
