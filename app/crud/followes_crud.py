...
def get_follower_count(db: Session, user_id: UUID) -> dict:
    ...
    """
    followers_count = (
        db.query(Follows).filter(Follows.creator_user_id == user_id).count()
    )
    following_count = (
        db.query(Follows).filter(Follows.follower_user_id == user_id).count()
    )
    return {"followers_count": followers_count, "following_count": following_count}


def create_follow(
    db: Session, follower_user_id: UUID, creator_user_id: UUID
) -> Follows:
    """
    フォロー関係を作成
    """
...
def get_followers(db: Session, user_id: UUID, skip: int = 0, limit: int = 20):
    """
    フォロワー一覧を取得（Users + Profilesの情報を返す）
    """
    return (
        db.query(Users.id, Users.profile_name, Profiles.username, Profiles.avatar_url)
...
def get_following(db: Session, user_id: UUID, skip: int = 0, limit: int = 20):
    """
...
def toggle_follow(db: Session, follower_user_id: UUID, creator_user_id: UUID) -> dict:
    ...
    if existing_follow:
        ...
        # フォロー
        follow = Follows(
            follower_user_id=follower_user_id, creator_user_id=creator_user_id
        )
        db.add(follow)
        db.commit()
...
def add_notification_follow(
    ...
    try:
        ...
        if not should_send_notification_follow:
            return
        follower_profile = (
            db.query(Profiles).filter(Profiles.user_id == follower_user_id).first()
        )
        ...
        if not follower_user:
            ...
            return
        display_name = (
            follower_user.profile_name
            if follower_user.profile_name
            else (follower_user.email or "ユーザー")
        )
        avatar_url = (
            f"{os.environ.get('CDN_BASE_URL')}/{follower_profile.avatar_url}"
            if (follower_profile and follower_profile.avatar_url)
            else f"{os.environ.get('FRONTEND_URL')}/assets/no-image.svg"
        )
...
def add_mail_notification_follow(
    db: Session, follower_user_id: UUID, creator_user_id: UUID
) -> None:
    """
    フォロー通知メールを追加
    ...
    try:
        user = (
            db.query(Users, Profiles, UserSettings.settings)
            .join(Profiles, Users.id == Profiles.user_id)
            .outerjoin(
                UserSettings,
                and_(
        ...
        send_follow_notification_email(
            ...
            f"{os.environ.get('FRONTEND_URL', 'https://mijfans.jp')}/profile?username={follower_profile.username}",
        )
    except Exception as e:
        db.rollback()
