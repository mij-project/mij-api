...
from app.crud.media_rendition_jobs_crud import (
    ...
    get_media_rendition_job_by_id,
)
from app.crud.media_assets_crud import (
    get_media_asset_by_id,
    update_media_asset,
    check_all_media_assets_approved,
)
...
MIME_TYPE_MAP = {".m3u8": "application/vnd.apple.mpegurl", ".mp4": "video/mp4"}
...
def _handle_preview_completion(db: Session, webhook_data: dict) -> None:
        ...
        if rendition_job.asset_id:
            ...
            if asset:
                ...
                update_post_status(
                    db,
                    asset.post_id,
                    PostStatus.CONVERT_FAILED,
                    AuthenticatedFlag.NOT_AUTHENTICATED,
                )
        return

    # 成功時の処理
    # マスターファイルを検索
    storage_key, size_bytes = _find_master_file(
        webhook_data["detail"], rendition_job.output_prefix
    )
    ...
    # レンディションジョブを更新
    update_data = {"status": webhook_data["status"], "output_key": storage_key}
...
def _handle_final_hls_completion(db: Session, webhook_data: dict) -> None:
    ...
    if all_media_approved:
        ...
        post = update_post_status(
            db, asset.post_id, PostStatus.APPROVED, AuthenticatedFlag.AUTHENTICATED
        )
        if not post:
        ...
        # 新着投稿通知をトリガー
        trigger_batch_notification_newpost_arrival(
            post_id=str(post.id), creator_user_id=str(post.creator_user_id)
        )


def _find_master_file(
    detail: dict, output_prefix: str
...
def _get_duration_with_ffmpeg(storage_key: str) -> Optional[Decimal]:
    ...
    try:
        ...
        with tempfile.NamedTemporaryFile(
            suffix=_get_file_extension(storage_key)
        ) as temp_file:
            # S3からファイルをダウンロード
