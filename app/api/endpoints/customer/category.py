...
async def get_category_by_slug(
    ...
    try:
        ...
        next_page, previous_page, has_next, has_previous = __process_pagination(posts, page, per_page)
        return PaginatedPostCategoryResponse(
            posts=[PostCategoryResponse(
                id=post.post_id,
                ...
                duration=get_video_duration(post.duration_sec) if post.post_type == PostType.VIDEO and post.duration_sec else ("画像" if post.post_type == PostType.IMAGE else ""),
            ) for post in posts],
            total=total,
            page=page,
            per_page=per_page,
