...
from app.crud.time_sale_crud import get_price_time_sale_by_id
from app.crud.post_crud import get_post_by_id
from app.schemas.post_plan_timesale import PlanTimeSaleEditInitResponse, PlanTimeSaleResponse
...
def get_post_price_timesale_edit_by_id(
    time_sale_id: UUID,
    ...
    try:
        ...
        post_detail = {
            "id": post.get("id", ""),
            "title": post.get("description", ""),  # get_post_by_idはtitleではなくdescriptionを返す
            "price": post.get("single_price", 0) or 0,  # single_priceは既に価格の値（整数）を返す
        }
        ...
        time_sale = PlanTimeSaleResponse(
            ...
            plan_id=str(time_sale_data.TimeSale.plan_id) if time_sale_data.TimeSale.plan_id else None,
