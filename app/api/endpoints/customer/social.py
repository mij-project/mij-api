...
def get_like_status(
    ...
    """いいね状態を確認"""
    is_liked = likes_crud.is_liked(db, post_id, current_user)
...
def get_likes_status_bulk(
    ...
    for post_id in post_ids:
        is_liked = likes_crud.is_liked(db, post_id, current_user)
        likes_count = likes_crud.get_likes_count(db, post_id)
        result[str(post_id)] = {"liked": is_liked, "likes_count": likes_count}
    return result
...
@router.get(
    "/comments/{parent_comment_id}/replies", response_model=List[CommentResponse]
)
...
def update_comment(
    ...
    if not updated_comment:
        raise HTTPException(
            status_code=404, detail="コメントが見つからないか、編集権限がありません"
        )
...
def delete_comment(
    ...
    if not success:
        raise HTTPException(
            status_code=404, detail="コメントが見つからないか、削除権限がありません"
        )
...
def get_bookmarks_status_bulk(
    ...
    for post_id in post_ids:
        is_bookmarked = bookmarks_crud.is_bookmarked(db, current_user.id, post_id)
        result[str(post_id)] = {"bookmarked": is_bookmarked}
    return result
