...
@router.get("/delusion/messages", response_model=DelusionMessagesResponse)
def get_delusion_messages(
    ...
    return DelusionMessagesResponse(
        messages=response,
        has_next=has_next,
    )
...
@router.get("/delusion-unread")
async def get_delusion_unread(
    current_user: Users = Depends(get_current_user), db: Session = Depends(get_db)
):
    """
    妄想メッセージの新着メッセージ数を取得
    """
    is_unread = conversations_crud.get_delusion_unread(db, current_user.id)
    return {"is_unread": is_unread}
...
def get_conversation_messages(
    ...
    """
    # ユーザーがこの会話に参加しているか確認
    if not conversations_crud.is_user_in_conversation(
    ...
    for message, sender, profile, admin in messages:
        ...
        # メッセージアセット情報を取得
        asset_response = None
        message_assets = message_assets_crud.get_message_assets_by_message_id(
            db, message.id
        )
        if message_assets:
            ...
            cdn_thumbnail_url = None
            if asset.thumbnail_storage_key:
                cdn_thumbnail_url = (
                    f"{MESSAGE_ASSETS_CDN_URL}/{asset.thumbnail_storage_key}"
                )
    ...
    if partner_user_id:
        # チップ送信履歴の確認（双方向）
        has_chip_history_to_partner = bool(
            payments_crud.get_payment_by_user_id(
                db, current_user.id, partner_user_id, PaymentType.CHIP
            )
        )
        has_chip_history_from_partner = bool(
            payments_crud.get_payment_by_user_id(
                db, partner_user_id, current_user.id, PaymentType.CHIP
            )
        )

        # DM解放プラン加入の確認（双方向）
        # current_userがpartner_userのDM解放プランに加入している（open_dm_flg=true）
        has_dm_plan_to_partner = bool(
            subscriptions_crud.get_dm_release_plan_subscription(
                db, current_user.id, partner_user_id
            )
        )
        # partner_userがcurrent_userのDM解放プランに加入している（open_dm_flg=true）
        has_dm_plan_from_partner = bool(
            subscriptions_crud.get_dm_release_plan_subscription(
                db, partner_user_id, current_user.id
            )
        )

        # どちらか一方を満たせばメッセージ送信可能
        can_send_message = (
            has_chip_history_to_partner
            or has_dm_plan_to_partner
            or has_dm_plan_from_partner
            or has_chip_history_from_partner
        )
    ...
    is_current_user_seller = (
        bool(
            subscriptions_crud.get_subscription_by_user_id(
                db, partner_user_id, current_user.id
            )
        )
        or has_chip_history_from_partner
...
def send_conversation_message(
    ...
    if not message_data.body_text and not message_data.asset_storage_key:
        raise HTTPException(
            status_code=400, detail="Either body_text or asset is required"
        )
    ...
    if message_data.asset_storage_key and not message_data.asset_type:
        raise HTTPException(
            status_code=400,
            detail="asset_type is required when asset_storage_key is provided",
        )
    ...
    if message_asset:
        ...
        try:
            ...
            # 送信者のプロフィール情報を取得
            sender_profile = (
                db.query(Profiles).filter(Profiles.user_id == current_user.id).first()
            )
            sender_avatar_url = (
            ...
            if message_data.body_text:
                message_preview = (
                    message_data.body_text[:50]
                    if len(message_data.body_text) > 50
                    else message_data.body_text
                )
            ...
            for recipient in recipients:
                ...
                if need_to_send_email_notification and recipient_user.email:
                    logger.info(
                        f"Attempting to send email notification to {recipient_user.email} for message {message.id}"
                    )
                    ...
                    logger.info(
                        f"Email notification call completed for {recipient_user.email}"
                    )
                else:
                    if not need_to_send_email_notification:
                        logger.info(
                            f"Email notification disabled for user {recipient_user.id}, skipping email notification"
                        )
                    else:
                        logger.info(
                            f"Recipient user {recipient_user.id} has no email address, skipping email notification"
                        )
...
def mark_conversation_message_as_read(
    ...
    logger.info(
        f"Mark as read request: conversation_id={conversation_id}, message_id={message_id}, user_id={current_user.id}"
    )
    ...
    if not conversations_crud.is_user_in_conversation(
        ...
        logger.warning(
            f"User {current_user.id} is not a participant in conversation {conversation_id}"
        )
        raise HTTPException(
            status_code=403, detail="Access denied to this conversation"
        )
...
@router.post(
    "/{conversation_id}/messages/upload-url", response_model=PresignedUrlResponse
)
def get_message_asset_upload_url(
    ...
    try:
        ...
        asset_type_str = (
            "image" if request.asset_type == MessageAssetType.IMAGE else "video"
        )
        ...
        logger.error(
            f"メッセージアセット用のPresigned URL取得エラーが発生しました: {e}"
        )
