...
from app.crud.plan_crud import (
    get_plan_by_user_id,
    get_single_purchases_by_user_id,
    get_user_plans,
    get_plan_monthly_sales,
)
...
from app.crud.profile_crud import (
    ...
    get_profile_info_by_user_id,
    get_profile_edit_info_by_user_id,
    update_profile,
...
def get_account_info(
    ...
    try:
        ...
        profile_info = ProfileInfo(
            ...
            avatar_url=f"{BASE_URL}/{profile_info_data['avatar_url']}"
            if profile_info_data["avatar_url"]
            else None,
            cover_url=f"{BASE_URL}/{profile_info_data['cover_url']}"
            if profile_info_data["cover_url"]
            else None,
        )
        ...
        posts_info = PostsInfo(
            ...
            reserved_posts_count=posts_data["reserved_posts_count"]
            if posts_data
            else 0,
        )
            ...
            single_purchases_data.append(
                {
                    ...
                    "creator_avatar_url": f"{BASE_URL}/{purchase.creator_avatar_url}"
                    if purchase.creator_avatar_url
                    else None,
                    "thumbnail_key": f"{BASE_URL}/{purchase.thumbnail_key}"
                    if purchase.thumbnail_key
                    else None,
        ...
        plan_info = PlanInfo(
            ...
            subscribed_plan_count=plan_data["subscribed_plan_count"]
            if plan_data
            else 0,
            subscribed_total_price=plan_data["subscribed_total_price"]
            if plan_data
            else 0,
...
def get_profile_edit_info(
    ...
    try:
        ...
        if not profile_data:
            raise HTTPException(
                status_code=404, detail="プロフィール情報が見つかりませんでした"
            )

        return ProfileEditInfo(
            profile_name=profile_data["profile_name"] or "",
            username=profile_data["username"] or "",
            avatar_url=f"{BASE_URL}/{profile_data['avatar_url']}"
            if profile_data.get("avatar_url")
            else None,
            cover_url=f"{BASE_URL}/{profile_data['cover_url']}"
            if profile_data.get("cover_url")
            else None,
...
def update_account_info(
    ...
    try:
        ...
        # 氏名の変更チェックと重複確認
        name_changed = (
            update_data.name and update_data.name != current_user.profile_name
        )
        if name_changed:
            logger.info(
                f"氏名変更を検出: {current_user.profile_name} -> {update_data.name}"
            )
            if check_profile_name_exists(db, update_data.name):
                logger.warning(f"氏名の重複エラー: {update_data.name}")
                return Response(
                    content="このユーザ名は既に使用されています", status_code=400
                )
        ...
        if username_changed:
            logger.info(
                f"ユーザーネーム変更を検出: {current_profile.username} -> {update_data.username}"
            )
            if exist_profile_by_username(db, update_data.username):
                logger.warning(f"ユーザーネームの重複エラー: {update_data.username}")
                return Response(
                    content="このユーザーネームは既に使用されています", status_code=400
                )
        ...
        instagram_link = (
            f"{INSTAGRAM_URL}/{instagram.replace('@', '')}" if instagram else ""
        )
        ...
        return AccountUpdateResponse(
            message="アカウント情報が正常に更新されました", success=True
        )
...
@router.get("/posts")
def get_post_status(user=Depends(get_current_user), db: Session = Depends(get_db)):
    """
    投稿ステータスを取得
    """
    try:
...
def get_account_post_detail(
    ...
    try:
        ...
        return AccountPostDetailResponse(
            ...
            reject_comments=post_base_info["post"].reject_comments
            if post_base_info["post"].reject_comments
            else None,
            ...
            creator_avatar_url=f"{BASE_URL}/{post_base_info['creator_avatar_url']}"
            if post_base_info.get("creator_avatar_url")
            else None,
            price=int(post_base_info["price"])
            if post_base_info["price"] is not None
            else 0,
            currency=post_base_info["currency"] or "JPY",
            scheduled_at=post_base_info["post"].scheduled_at.isoformat()
            if post_base_info["post"].scheduled_at
            else None,
            expiration_at=post_base_info["post"].expiration_at.isoformat()
            if post_base_info["post"].expiration_at
            else None,
...
@router.get("/plans")
def get_plans(current_user=Depends(get_current_user), db: Session = Depends(get_db)):
    """
    プラン一覧を取得
    """
    try:
        ...
        return PlansSubscribedInfo(
            ...
            subscribed_plan_names=plan_data["subscribed_plan_names"]
            if plan_data
            else [],
            subscribed_plan_details=subscribed_plan_details,
        )
...
def submit_profile_image(
    ...
    try:
        ...
        existing_submission = (
            profile_image_crud.get_pending_submission_by_user_and_type(
        ...
        slack_alert._alert_profile_verification(
...
def _convert_posts(posts_list):
    ...
    for row in posts_list:
        ...
        # 投稿タイプを判定
        is_video = (
            post_obj.post_type == 1 if post_obj.post_type else False
        )  # PostType.VIDEO = 1
...
def update_account_post(
    ...
    try:
        ...
        if not updated_post:
            raise HTTPException(
                status_code=404, detail="投稿が見つからないか、更新する権限がありません"
            )

        db.commit()

        return AccountPostUpdateResponse(message="投稿を更新しました", success=True)
...
async def setting_email(
    ...
    try:
        if email_setting.type == 1:
            ...
            email_existing = (
                db.query(Users).filter(Users.email == email_setting.email).first()
            )
            if email_existing or (current_user.email == email_setting.email):
                return Response(
                    content="既に使用されているメールアドレスです。", status_code=400
                )
            token = __generate_email_verification_token(
                db, email_setting.email, current_user.id
            )
            if not token:
                raise HTTPException(
                    status_code=500,
                    detail="メールアドレスの認証トークンの生成に失敗しました",
                )
            verify_url = (
                f"{os.environ.get('FRONTEND_URL')}/setting/verify-email?token={token}"
            )
            send_email_verification(
                to=email_setting.email,
            ...
            if exp_dt < datetime.now(timezone.utc):
                return Response(
                    content="リンクが無効か、期限切れです。", status_code=400
                )
...
def __generate_email_verification_token(db: Session, email: str, user_id: UUID) -> str:
    ...
    payload = {"user_id": str(user_id), "email": email, "exp": exp_time}
