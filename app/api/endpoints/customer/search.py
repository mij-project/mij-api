...
def get_search_categories(
    ...
    try:
        ...
        items = [
            SearchCategoryItem(id=category.id, name=category.name, slug=category.slug)
...
def search(
    ...
    sort: str = Query(
        "relevance", regex="^(relevance|popularity)$", description="ソート基準"
    ),
    category_ids: Optional[List[UUID]] = Query(None, description="カテゴリフィルター"),
    post_type: Optional[int] = Query(
        None, ge=1, le=2, description="投稿タイプ (1=VIDEO, 2=IMAGE)"
    ),
    ...
    try:
            ...
            for r in creators_results:
                ...
                if hasattr(r, "recent_posts"):
                    recent_posts = [
                        {
                            "id": p["id"],
                            "thumbnail_url": f"{BASE_URL}/{p['thumbnail_url']}"
                            if p.get("thumbnail_url")
                            else None,
                            "is_time_sale": p.get("is_time_sale", False),
                        }
                ...
                creators_items.append(
                    CreatorSearchResult(
                        ...
                        username=r.username,
                        avatar_url=f"{BASE_URL}/{r.avatar_url}"
                        if r.avatar_url
                        else None,
                        bio=r.bio,
                        ...
                        official=r.offical_flg
                        if hasattr(r, "offical_flg") and r.offical_flg is not None
                        else False,
        ...
        if type in ["all", "posts", "paid_posts"]:
            ...
            posts_items = [
                PostSearchResult(
                    ...
                    thumbnail_key=f"{BASE_URL}/{r.thumbnail_key}"
                    if r.thumbnail_key
                    else None,
        ...
        if type in ["all", "hashtags"]:
            ...
            hashtags_items = [
                HashtagSearchResult(
                    id=r.id, name=r.name, slug=r.slug, posts_count=r.posts_count
                )
...
def delete_all_search_history(
    ...
    検索履歴全削除
    """
    search_history_crud.delete_all_search_histories(db, user_id=current_user.id)
